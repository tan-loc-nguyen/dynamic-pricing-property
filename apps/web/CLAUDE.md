...
- `components/NavShell.tsx` + `components/Nav.tsx` — the left rail, on shadcn's `Sidebar` in `collapsible="icon"` mode. `NavShell` exists for one reason: `SidebarProvider` persists the collapsed state to a cookie that a normal Next app reads back in the *server* layout, and this app is exported statically (D32) — so it was written and never restored. `NavShell` reads it on the client after mount.
...
Mobile responsiveness is out of scope by explicit decision, not an oversight. This is a working tool for a revenue manager at a desk (same category of decision as "no dark mode"), and doing it properly is a full mobile-nav-pattern undertaking, not a quick patch. Don't attempt a partial fix (e.g. just hiding the sidebar) without raising it as its own piece of work.

The rail itself is no longer the problem it was: it collapses to 48px, so a narrow window is cramped rather than unusable. What is still missing is everything else — page layouts, tables and the drawer all assume desktop width.

**shadcn's Sidebar ships a mobile path that has been deliberately removed** from `components/ui/sidebar.tsx`: below 768px it unmounted the rail and re-offered it inside a `Sheet` that only `SidebarTrigger` can open — and this app's trigger lives *inside* the rail. The result was a narrow window with no rail, no trigger and no reachable navigation, which is exactly the partial fix warned against above. Both `md:` gates and the `Sheet` branch are gone and the rail renders at every width. If you re-run `npx shadcn add sidebar`, that comes back — check the diff.
